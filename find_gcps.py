...
parser.add_argument('--ref_mask_fnam',default=REF_MASK_FNAM,help='Reference mask file name (%(default)s)')
parser.add_argument('--trg_ndvi',default=False,action='store_true',help='Target is NDVI with R=trg_multi_band[0] and NIR=trg_multi_band[1] (%(default)s)')
...
def interp_img(p,refx,refy,refz,refm,trgx,trgy,trgz):
    if args.trg_blur_sigma is not None:
        f = RegularGridInterpolator((trgx+p[0],trgy[::-1]+p[1]),gaussian_filter_nan(trgz,sigma=args.trg_blur_sigma)[::-1].T,method=args.interp)
    else:
...
if args.ref_mask_fnam is not None:
    ds = gdal.Open(args.ref_mask_fnam)
...
if args.trg_multi_band is not None:
    if args.trg_ndvi:
        ...
        if args.trg_data_min is not None:
            ...
            cnd = (nir < args.trg_data_min)
            trg_data[cnd] = nir[cnd]
        if args.trg_data_max is not None:
            cnd = (red > args.trg_data_max)
            trg_data[cnd] = red[cnd]
            cnd = (nir > args.trg_data_max)
            trg_data[cnd] = nir[cnd]
    else:
...
for trg_indyc in np.arange(args.trg_indy_start,args.trg_indy_stop,args.trg_indy_step):
    ...
    for trg_indxc in np.arange(args.trg_indx_start,args.trg_indx_stop,args.trg_indx_step):
        ...
        #if args.trg_data_min is not None and trg_subset_data.min() < args.trg_data_min:
        #    continue
        #if args.trg_data_max is not None and trg_subset_data.max() > args.trg_data_max:
        #    continue
        # reference subset
        ...
        if args.ref_data_min is not None and np.nanmin(ref_subset_data) < args.ref_data_min:
            continue
