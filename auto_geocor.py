...
parser.add_option('-I','--interp',default=None,help='Interpolation method (%default)')
...
parser.add_option('--ref_mask_fnam',default=None,help='Reference mask file name (%default)')
...
parser.add_option('--optfile',default=None,help='Option file name for gdal_translate (%default)')
...
if opts.use_gcps is not None:
    ...
    if opts.ref_data_umax is not None:
        command += ' --ref_data_umax {}'.format(opts.ref_data_umax)
    if opts.ref_data_umin is not None:
        command += ' --ref_data_umin {}'.format(opts.ref_data_umin)
    if opts.trg_blur_sigma is not None:
        command += ' --trg_blur_sigma {}'.format(opts.trg_blur_sigma)
    if opts.interp is not None:
        command += ' --interp {}'.format(opts.interp)
    ...
    if opts.scan_fnam is not None:
        command += ' --scan_fnam {}'.format(opts.scan_fnam)
    if opts.ref_mask_fnam is not None:
        command += ' --ref_mask_fnam {}'.format(opts.ref_mask_fnam)
    if opts.trg_ndvi:
        command += ' --trg_ndvi'
    if opts.exp:
        command += ' --exp'
...
if trg_fnam is not None:
    ...
    if opts.optfile is not None:
        with open(opts.optfile,'w') as fp:
            fp.write(line)
        command += ' --optfile {}'.format(opts.optfile)
    else:
        command += line
    command += ' '+trg_fnam
    command += ' '+tmp_fnam
    ...
    if opts.optfile is not None:
        if os.path.exists(opts.optfile):
            os.remove(opts.optfile)
