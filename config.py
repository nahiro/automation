...
'identify.geocor_fnam'                : os.path.join(main_drone_analysis,'Current','geocor','orthomosaic_resized_geocor.tif'),
...
for proc in pnams:
    ...
    modules[proc].field_data = config['main'].get('main.field_data')
    modules[proc].drone_data =  config['main'].get('main.drone_data')
    modules[proc].drone_analysis = config['main'].get('main.drone_analysis')
